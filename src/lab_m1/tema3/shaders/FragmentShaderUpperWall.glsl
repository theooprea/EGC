...
uniform vec3 light_positions[9];
uniform vec3 light_colours[9];
uniform vec3 eye_position;

uniform vec3 light_directions_spotlight[4];
uniform vec3 light_positions_spotlight[4];
uniform vec3 light_colours_spotlight[4];
...
void main()
{
    ...
    for (int i = 0; i < 9; i++) {
        ...
        if (d < 1) {
	        attenuation_factor = pow(1 - d, 2);
        }

	    vec3 light_intensity = ambient_light + attenuation_factor * (diffuse_light + specular_light);

        light_intensity_final += light_intensity;
    }

    for (int i = 0; i < 4; i++) {
        vec3 light_position = light_positions_spotlight[i];
        vec3 light_direction = light_directions_spotlight[i];
        vec3 light_colour = light_colours_spotlight[i];
    ...
    vec3 colour = light_intensity_final;
